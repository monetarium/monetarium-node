...
import (
	...
	"github.com/monetarium/monetarium-node/internal/mempool"
	"github.com/monetarium/monetarium-node/internal/mining"
	"github.com/monetarium/monetarium-node/internal/mining/cpuminer"
	"github.com/monetarium/monetarium-node/internal/netsync"
	"github.com/monetarium/monetarium-node/internal/rpcserver"
...
func (ps *peerState) ResolveLocalAddress(netType addrmgr.NetAddressType, addrMgr *addrmgr.AddrManager, services wire.ServiceFlag) {
	...
	addLocalAddress := func(bestSuggestion string, port uint16, services wire.ServiceFlag) {
		na, err := hostToNetAddress(bestSuggestion, port, services, mondLookup)
		if err != nil {
...
func (sp *serverPeer) handleServeGetData(invVects []*wire.InvVect,
	...
	for _, iv := range invVects {
		...
		case wire.InvTypeBlock:
			...
			// longer used in mond which is now based on a much more robust
...
// attemptMondDial is a wrapper function around mondDial which adds and marks
// the remote peer as attempted in the address manager.
func (s *server) attemptMondDial(ctx context.Context, network, addr string) (net.Conn, error) {
	if !cfg.SimNet && !cfg.RegNet {
		...
		remoteAddr, err := hostToNetAddress(host, uint16(port), 0, mondLookup)
		if err != nil {
			return nil, err
		}
	...
	return mondDial(ctx, network, addr)
}
...
func (s *server) handleBlockchainNotification(notification *blockchain.Notification) {
	...
	case blockchain.NTBlockConnected:
		...
		// Note: dual-coin fee calculator recording happens at mempool acceptance
		// (see TxPool.addTransaction). A previous block-connect recording site
		// double-counted self-mined txs on high-hashrate nodes and was removed.
...
// newServer returns a new mond server configured to listen on addr for the
...
func initListeners(ctx context.Context, params *chaincfg.Params, amgr *addrmgr.AddrManager, listenAddrs []string, services wire.ServiceFlag) ([]net.Listener, *upnpNAT, error) {
	...
	if len(cfg.ExternalIPs) != 0 {
		...
		for _, sip := range cfg.ExternalIPs {
			...
			na, err := hostToNetAddress(host, eport, services, mondLookup)
			if err != nil {
...
func addrStringToNetAddr(addr string) (net.Addr, error) {
	...
	// The mondLookup function will transparently handle performing the
	// lookup over Tor if necessary.
	ips, err := mondLookup(host)
	if err != nil {
		return nil, err
	}
	if len(ips) == 0 {
