...
configure_mining_and_voting() {
    if ! $MINING_ENABLED && ! $TICKETS_ENABLED; then
        return
    fi

    if ! has_tty; then
        info "Non-interactive mode — skipping wallet address polling."
        return
    fi
    ...
    if [[ -n "$CONSOLIDATION_ADDR" ]]; then
        ...
        case "$(uname -s)" in
            Linux)
                ...
                ;;
            Darwin)
                if launchctl kickstart -k "gui/$(id -u)/com.monetarium.wallet" 2>/dev/null; then
                    :
                else
                    launchctl unload "$HOME/Library/LaunchAgents/com.monetarium.wallet.plist" 2>/dev/null || true
                    launchctl load "$HOME/Library/LaunchAgents/com.monetarium.wallet.plist"
                fi
                ;;
        esac
    fi
...
ticket buyer:
  enabled:                  true
  limit:                    ${TICKET_LIMIT}
  balance to maintain:      ${TICKET_BALANCE}
  fee consolidation addr:   ${CONSOLIDATION_ADDR}
