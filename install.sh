...
configure_mining_and_voting() {
    ...
    {
        echo ""
        echo "; Staking configuration — added post-install by install.sh"
        echo "enablevoting=${vote_enabled}"
        echo "enableticketbuyer=${tb_enabled}"
        echo "ticketbuyer.limit=${tkt_limit}"
        echo "ticketbuyer.balancetomaintainabsolute=${tkt_balance}"
    } >> "$WALLET_CONF" 2>/dev/null || true
