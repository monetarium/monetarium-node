...
import (
	...
	"github.com/monetarium/monetarium-node/internal/progresslog"
)
...
func removeRegressionDB(dbPath string) error {
	...
	if err == nil {
		mondLog.Infof("Removing regression test database from '%s'", dbPath)
...
func warnMultipleDBs() {
	...
	if len(duplicateDbPaths) > 0 {
		...
		mondLog.Warnf("WARNING: There are multiple block chain databases "+
...
func loadBlockDB(params *chaincfg.Params) (database.DB, error) {
	...
	if cfg.DbType == "memdb" {
		mondLog.Infof("Creating block database in memory.")
	...
	mondLog.Infof("Loading block database from '%s'", dbPath)
	...
	if err := blockchain.CheckDBTooOldToUpgrade(db); err != nil {
		...
		mondLog.Infof("Removing database from '%s': %v", dbPath, err)
	...
	mondLog.Info("Block database loaded")
	return db, nil
}
...
func dumpBlockChain(params *chaincfg.Params, b *blockchain.BlockChain) error {
	mondLog.Infof("Writing the blockchain to flat file %q.  This might take a "+
		"while...", cfg.DumpBlockchain)

	progressLogger := progresslog.New("Wrote", mondLog)
