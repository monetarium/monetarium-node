...
mond provides full support for anonymous networking via the
...
[stream isolation](#TorStreamIsolation).  In addition, mond supports a hybrid,
...
Configuring mond as a Tor client is straightforward.  The first step is
obviously to install Tor and ensure it is working. Once that is done, all that
typically needs to be done is to specify the `--proxy` flag via the mond command
line or in the mond configuration file.  Typically the Tor proxy address will be
...
By default, mond assumes the proxy specified with `--proxy` is a Tor proxy and
...
discuss since mond will act as the server.
...
```text
HiddenServiceDir /var/tor/mond
HiddenServicePort 9508 127.0.0.1:9508
```

Once Tor is configured to provide the hidden service and you have obtained your
generated .onion address, configuring mond as a Tor hidden service requires
three flags:
...
```bash
$ ./mond --proxy=127.0.0.1:9050 --listen=127.0.0.1 --externalip=fooanon.onion
```
...
mond provides support for operating as a bridge between regular nodes and hidden
...
by using the `--onion` flag via the mond command line or in the mond
...
```bash
$ ./mond --onion=127.0.0.1:9050 --externalip=fooanon.onion
```
...
mond provides support for Tor stream isolation by using the `--torisolation`
...
```bash
$ ./mond --proxy=127.0.0.1:9050 --torisolation
```
